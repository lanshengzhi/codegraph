...
| "Trace a lifecycle / entry-to-target path" | \`codegraph_trace\` (static candidate with edge evidence, static ranking score/reason, and boundary / low-evidence caveats; not runtime proof) |
| "Give me focused context for a task/area" | \`codegraph_context\` (entry points may include static relevance reasons) |
| "See several related symbols' source at once" | \`codegraph_explore\` (file headers may include static relevance reasons) |
...
- **Treat edge evidence and trace/context/explore ranking as static.** \`codegraph_trace\`, \`codegraph_callers\`, and \`codegraph_callees\` show edge kind, source syntax evidence (such as \`direct-call\`, \`property-call\`, \`constructor-call\`, \`import\`, \`decorator\`, or \`bare-call\`) or resolver fallback (\`name-match\`, \`framework\`, \`fuzzy\`), callsite, provenance, confidence, and resolver when recorded. Trace paths may also show a static score/reason from recorded signals (direct-call ratio, edge confidence, scope, low-evidence, optional/test/generated penalties) plus boundary / low-evidence entries with exact handles to inspect next. \`codegraph_context\` / \`codegraph_explore\` may show relevance reasons from recorded search/graph signals (exact name/path matches, entry-point proximity, generic-name or test/generated penalties). \`not-recorded\` means CodeGraph did not capture that fact; static score/reasons are sorting guidance, not runtime main-path proof or complete semantic proof.
