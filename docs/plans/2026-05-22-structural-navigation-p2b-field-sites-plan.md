...
- **大 repo 性能安全阀**:analyzer 内部设 `maxFilesToParse`(默认 5000)与 batch 并发读取/parse(batch size 8),防止 monorepo 串行 I/O 阻塞;超出的文件 skip 为 `too-many-files`,状态降为 `partial`。
- **top-down traversal + 父节点决策**:assignment / mapping / return-object-field 的 dedupe 在父节点层面判断(例如 `return { systemPrompt }` 由 `return_statement` 直接输出 `return-object-field`,不再由子节点输出 `object-literal-key`)。
...
Checkpoint 0  AST shape probe / golden fixture matrix
  → 补充：type-only subtree shape、destructuring pattern node types、private_property_identifier、class field definition、JSX attribute
  → Checkpoint 1  Types + analyzer skeleton + source scanning guards
    → 修正：FieldSitesResult 补 request metadata（scopePath、includeTests、limit）
    → 修正：getFieldSites() 外层只过滤 scope/includeTests，analyzer 内做 language filter
    → 新增：maxFilesToParse（默认 5000）+ batch 并发读取/parse
    → 明确：top-down traversal + type-only skip + parse-error 策略（rootNode.hasError 整体 skip）
    → Checkpoint 2  property assignment/read + exact matching
      → 修正：AST matcher 包含 shorthand_property_identifier_pattern / pair_pattern / private_property_identifier
      → Checkpoint 3  object literal / destructuring / return object
        → Checkpoint 4  mapping hints + caveats
          → Checkpoint 5  lazy enclosing handles + ranking/grouping/statistics
            → Checkpoint 6  MCP tool + formatter + permissions + invalid arg tests
              → Checkpoint 7  三处 instructions 同步 + .cursor/rules/codegraph.mdc + README + CHANGELOG
```
...
export interface FieldSite {
  ...
  /** Access semantics for this site; compound/update writes use readwrite. construction/mapping 语义由 category 承载,access 只保留读写侧。 */
...
export interface FieldSitesResult {
  ...
  field: string;
  /** Normalized request options for formatter/debugging. */
  scopePath?: string;
  includeTests: boolean;
  limit: number;
  sites: FieldSite[];
...
async getFieldSites(field: string, options?: FieldSitesOptions): Promise<FieldSitesResult> {
  ...
  // 外层只做 scope/includeTests 过滤;语言过滤在 analyzer 内部完成,
  // 这样 searchedFiles / searchableFiles / skippedSummary.unsupported-language 统计才正确。
  const scoped = filterFilesByScopeAndTests(files, options);
...
1. **数量安全阀**:若已 parse 文件数达到 `maxFilesToParse`(默认 5000),剩余 supported files 直接 skip 为 `too-many-files`,状态降为 `partial`。该上限只约束实际 parse 数量,不影响统计口径。
2. **语言过滤**:仅 TS/JS/TSX/JSX 进入 parser;其他语言固定计入 `skippedSummary.unsupported-language` / `skippedFileCount`,但 `skippedFiles` 只保留 capped sample。此步骤在 analyzer 内部完成,因此外层 `getFieldSites()` 不得先按 language 过滤 file list。
3. `validatePathWithinRoot(projectRoot, file.path)`;失败则 skip `outside-root`。
4. `fs.stat()`;非 regular file 或 missing 则 skip `source-unavailable`。
5. size guard:超过 `maxSourceBytes`(默认建议 1 MiB)则 skip `source-too-large`。
6. `fs.readFile()`(batch 并发读取,batch size 8,避免串行 I/O 阻塞)。
7. `hashContent(source) !== file.contentHash` 则 skip `source-stale`,不 parse。
8. cheap prefilter:如果 source 不包含 field 字符串,且不包含 quoted field 可能形态,则 skip parse。注意 source prefilter 只能跳过明显无关文件,不能替代 AST exact match;如果 supported files 都被 prefilter 排除,状态应为 `no-matches`,并输出 caveat:`No source text contained the exact field string; dynamic/computed/alias cases are not covered.`
9. `await loadGrammarsForLanguages(distinctLanguages)` 应在文件循环前完成,或对每个 language 缓存 load/getParser 结果;parser 缺失则相关文件 skip `parser-unavailable`。
10. `parser.parse(source)`,`finally tree?.delete()`;每个实际进入 `parser.parse` 的文件计入 `parsedFiles`。
11. **parse error 策略**:若 `tree.rootNode.hasError` 为 true,整文件保守 skip 为 `parse-error`,状态至少为 `partial`,避免输出低置信 AST 坐标。
12. **type-only skip**:traversal 中遇到 `type_annotation`、`interface_body`、`property_signature`、`object_type`、`type_arguments`、`literal_type` 等 type-only subtree 直接跳过,不产 field site。P2b 目标是读写/构造/mapping,不是类型声明。
13. AST traversal(top-down,父节点决策)收集 sites;P2b 首版不做 collection hard cap,因此遍历完成后的 `totalSites*` 统计是精确值。
14. 如果该文件产生 sites,才调用 `loadNodesForFile(file.path)` 找 enclosing node:选择包含 `site.range.startLine` 的最小 range;优先 function/method,再 class/component/module/file。

File count 与 skipped 统计规则:

- `searchedFiles` = scope/includeTests 过滤后被考虑的 indexed files(外层不过滤 language)。
...
- `all-skipped`:存在 supported candidate files,但全部因 stale、too-large、unavailable、outside-root、parse-error、too-many-files 等原因被跳过;`skippedSummary` 必须说明原因分布。
...
- object pattern destructuring、alias destructuring、parameter destructuring(含 `shorthand_property_identifier_pattern`、`pair_pattern`);
- return object field;
- TS public class field / JS field definition(`public_field_definition` / `field_definition`)附近的 object/member syntax;
- private field(`private_property_identifier`)exact match;
- type-only syntax 不产 site(`interface`、`type alias`、`property_signature` 等);
- JSX/TSX attribute 不误报(`<Component systemPrompt={value} />` 明确列为非目标)。
...
- identifier / property_identifier / shorthand_property_identifier / `shorthand_property_identifier_pattern` / `pair_pattern` / `private_property_identifier` exact text;
- private field 首版按原 text 匹配(`#foo`),不做 `foo` fallback;
...
**跳过 type-only subtree**:traversal 中进入 `type_annotation`、`interface_body`、`property_signature`、`object_type`、`type_arguments`、`literal_type` 等节点时不再深入,避免 `interface Options { systemPrompt: string }` 被误报为 read/write site。
...
### Type-only syntax 跳过(新增规则)

P2b 目标是 field read/write/construction/mapping sites,不是类型系统声明。以下 syntax 不产 field site:

- `interface Options { systemPrompt: string }`
- `type T = { systemPrompt?: string }`
- `function f(x: Pick<Options, 'systemPrompt'>) { ... }`
- class property signature:`class Foo { systemPrompt: string }`(注意与 field definition `systemPrompt = 'x'` 区分)

实现中 traversal 遇到以下节点类型应跳过子树:
- `type_annotation`、`interface_body`、`property_signature`
- `object_type`、`type_arguments`、`literal_type`
- `type_alias_declaration`、`interface_declaration`

例外:value-level 语法即使位于 type-only 上下文中也不应被提前误杀。例如 `function f(x: Options) { return x.systemPrompt }` 中的 `x.systemPrompt` read 仍需识别。
...
access: 'read'  // construction 语义由 category 承载;access 表示 local shorthand 暗含 read
...
category: 'construction'
access: 'read'  // construction 语义由 category 承载
...
kind: 'field-mapping'
category: 'mapping'
access: 'read'  // mapping 语义由 category 承载
...
- 增加 AST shape / golden fixture matrix:assignment、compound/update、optional chain、computed string key、object shorthand、object pattern alias(`pair_pattern`)、parameter destructuring、return object、TSX expression。
- **边界 fixture**:
  - type-only false positive:`interface Options { systemPrompt: string }`、`type T = { systemPrompt?: string }`、`function f(x: Pick<Options, 'systemPrompt'>)` 不应产 site。
  - private field:`this.#systemPrompt` 对 field `#systemPrompt` 匹配,对 `systemPrompt`(无 `#`)不匹配。
  - class field definition:`class Foo { systemPrompt = 'default' }` 应识别为 write-like site。
  - JSX attribute:`<Component systemPrompt={value} />` 明确不产 site,测试验证不误报。
- 复用 existing grammar init pattern;parser-unavailable 使用 injected parserHost seam 稳定测试。
- 测试 guard 沿用当前 `node:sqlite` 模式(`HAS_SQLITE`),与 `addressability.test.ts` / `node-structure.test.ts` 保持一致。
...
- parse-error fixture:含 syntax error 的文件整体 skip,状态至少 `partial`,`skippedSummary.parse-error` 正确计数。
...
- `src/types.ts` 增加 FieldSites types(含 request metadata:`scopePath`、`includeTests`、`limit`)。
...
- **大 repo 性能**:`maxFilesToParse`(默认 5000)+ batch 并发读取/parse(batch size 8);超出文件 skip 为 `too-many-files`。
- **Traversal 策略**:top-down + 父节点决策;assignment/mapping/return-object 在父节点层面统一判断,避免 same syntax 重复输出。
- **Type-only skip**:traversal 中遇到 `type_annotation`、`interface_body`、`property_signature`、`object_type`、`type_arguments`、`literal_type` 等跳过子树。
- **Parse error 策略**:`rootNode.hasError` → 整文件 skip 为 `parse-error`,状态至少 `partial`。
...
- **private field**:query `#systemPrompt` 时 `this.#systemPrompt = 'new'` 为 assignment,`this.#systemPrompt` read 为 property-read;query `systemPrompt`(无 `#`)时不匹配 `#systemPrompt`。
...
- invalid args:missing field、non-string field、bad scopePath、bad limit、invalid `includeTests`(非 boolean)返回 MCP error。
...
- 输出走 `truncateOutput()`。
- **installer permissions**:`src/installer/targets/shared.ts#getCodeGraphPermissions()` 新增 `mcp__codegraph__codegraph_field_sites`。
- **installer coverage**:`__tests__/installer-targets.test.ts` 补测 permissions 列表包含新工具。
...
- `.cursor/rules/codegraph.mdc` 同步断言:包含 `codegraph_field_sites` 与 caveat 文案。
...
- **`.cursor/rules/codegraph.mdc` 强制更新**:rebase 后该文件已存在(42 行,带 `<!-- CODEGRAPH_START -->` 标记),必须在其中新增 `codegraph_field_sites` 推荐规则。
- **installer targets 文案**:当前 registry 已有 5 个 target(Claude、Cursor、Codex、opencode、Hermes),README/installer 文案不要回退成 4-target 叙述。
...
- [ ] AST shape / golden fixture matrix 覆盖 optional chain、computed string key、object pattern alias（`pair_pattern`）、parameter destructuring、TSX expression、private field（`private_property_identifier`）、class field definition。
- [ ] type-only false positive 有测试：`interface`、`type alias`、`property_signature` 不产 site；value-level read 位于 type 上下文中仍应识别。
...
- [ ] `src/mcp/server-instructions.ts`、`src/installer/instructions-template.ts`、`.cursor/rules/codegraph.mdc`、README（和必要时 CHANGELOG）同步更新。
- [ ] installer permissions（`shared.ts#getCodeGraphPermissions()`）包含 `mcp__codegraph__codegraph_field_sites`，并有 `__tests__/installer-targets.test.ts` 覆盖。
...
**缓解：** 仅扫描 indexed TS/JS files；先 size guard，再 cheap `source.includes(field)` prefilter，再 parse；支持 `scopePath`；默认 `limit`；输出 skipped/omitted summary。analyzer 内部设 `maxFilesToParse`（默认 5000）与 batch 并发读取/parse（batch size 8），防止 monorepo 串行 I/O 阻塞；超出的文件 skip 为 `too-many-files`，状态降为 `partial`。若后续仍慢，再考虑 persistent field index 或 FTS source cache，但不放入 P2b 首版。
