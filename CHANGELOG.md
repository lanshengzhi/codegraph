...
### Added
- **MCP / registry candidates**: New `codegraph_registry_candidates` tool lists static registry/resolver candidates (provider, tool, extension, route, handler) discovered from indexed TypeScript/JavaScript files via AST pattern matching. Detects object-literal registries, Map constructors, `.set()` registrations, register-like calls, and definition arrays. Each candidate carries `evidence`, `confidence`, exact source `range`, and handler node resolution when handlers are indexed. Output includes static-candidate caveats and compact next checks (`codegraph_node`, `codegraph_search`). Registry candidates are static only — not runtime branch proof.
- **MCP / workspace import candidates**: New `codegraph_import_candidates` tool lists source entry candidates for workspace package imports (e.g., `@scope/pkg` or `@scope/pkg/subpath`). It discovers packages from `package.json` workspaces or `pnpm-workspace.yaml`, reads `exports` / `main` / `module` / `types` fields, and falls back to conventional paths (`src/index.ts`, `index.ts`, subpath conventions). Each candidate carries `evidence` (exports-exact, exports-condition, main-field, src-index-convention, dist-to-src-heuristic, re-export-chain), `confidence`, and `indexed` status. Pass `symbol` to chase a named export through barrel re-export chains and receive an exact `nodeId` handle. Output includes static-candidate caveats and copyable next checks (`codegraph_node`, `read`).
- **Resolver / workspace packages**: The import resolver now recognizes workspace package specifiers when there is a single high-confidence indexed source candidate (`confidence >= 0.8`). This lets `codegraph_trace`, `codegraph_callers`, and `codegraph_callees` follow edges across workspace package boundaries without misclassifying local workspace packages as external npm packages. Ambiguous or unindexed workspace packages remain unresolved — use `codegraph_import_candidates` to inspect candidates.
- **MCP / status coverage**: `codegraph_status` now supports `detail: "coverage"` for indexed-source boundary explanations, including pending changes, extraction errors, unresolved reference summaries, workspace package hints, tsconfig/jsconfig path-alias summaries, and an optional filesystem comparison (`checkFilesystem: true`). Default `detail: "summary"` remains compact and backward-compatible. Coverage output explicitly states that CodeGraph reports indexed source coverage, not a complete filesystem inventory.
- **CLI / status coverage**: `codegraph status --coverage` and `codegraph status --check-filesystem` expose the same coverage report in terminal and JSON output.
- **MCP / field sites**: New `codegraph_field_sites` tool for finding static AST-level read, write, object-construction, destructuring, return-object, and mapping-hint locations for a field/key name in TypeScript/JavaScript files. Results are grouped by category (Writes → Mapping hints → Object construction → Reads) with exact ranges, enclosing node handles, and next-step recommendations. Field sites are syntax-level navigation hints, not full dataflow, alias analysis, or runtime payload proof.
...
- **MCP / edge evidence**: TypeScript/JavaScript call, constructor, import, and decorator references now carry recorded source-shape evidence through resolution, so trace/callers/callees can show `evidence=direct-call`, `property-call`, `constructor-call`, `import`, `decorator`, or resolver fallbacks like `name-match` when available instead of defaulting to `not-recorded`.
- **MCP / trace boundaries**: `codegraph_trace` now reports structured boundaries and low-evidence edges for incomplete or weak static paths, including max-depth frontiers, indexed dead ends, framework/static-resolution caveats, metadata-not-recorded edges, and exact `codegraph_node` / `codegraph_callees` / `read` follow-ups. Missing call-shape evidence stays explicit as `not-recorded` / unclassified instead of guessing callback, property-call, or registry bindings.
- **MCP / trace ranking**: `codegraph_trace` now over-collects candidate paths before sorting and shows a static ranking score/reason for each path. Ranking uses recorded evidence such as direct-call ratio, edge confidence, low-evidence edges, optional-branch keywords, and test/fixture/generated penalties, while explicitly stating that the score is not runtime main-path proof.
- **MCP / context and explore reasons**: `codegraph_context` entry points and `codegraph_explore` file sections now show compact static relevance reasons when recorded, such as exact symbol/path matches, entry-point graph proximity, and generic-name or test/generated penalties. Missing reason data is shown as `not recorded` instead of inventing an explanation.

### Changed
- **MCP / tool names**: MCP tool definitions are now advertised as raw suffix names (`search`, `status`, `node`, …) with prefixed aliases (`codegraph_search`, `codegraph_status`, …) accepted internally for compatibility with Pi and other global tool gateways. This avoids collisions in clients that flatten tools into one namespace while keeping the names agents actually invoke short and consistent.
- **MCP / search schema**: `codegraph_search.kind` now exposes the real `NodeKind` values (`type_alias`, `constant`, `field`, etc.) instead of a smaller hand-written list that could steer agents toward invalid filters.
- **MCP / node structure**: `codegraph_node({ detail: "structure" })` now returns a compact member outline for container symbols (classes/interfaces/structs/enums) instead of an unsupported-node-kind response.
- **Agent guidance**: Instructions now explicitly say CodeGraph is not a git diff tool; agents should use `git status` / `git diff` for working-tree changes before using CodeGraph for structural impact.
...
- **Extraction / TypeScript and JavaScript**: Plain class fields are indexed as `field` nodes, while arrow-function class fields remain callable `method` nodes, so class outlines and call-graph queries no longer imply data fields are methods.
